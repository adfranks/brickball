...
<head>
  ...
  content="breakout game for web browser change email page">
...
if ($_SESSION['pword'] == md5($_POST['currentpwd'])) {
    try {
        ...
        echo '<p class="form-response">Email successfully updated!' .
        ...
        '</span><br><br><a class="error-link" href=' .
        '"brickball.php">Go back to the game.</a></p>';
    }

    $conn = null;
} else {
    echo '<div class="form-response"><span class="errormsg">' .
    'The password entered was incorrect.  Please, try again.</span>' .
    '<form class="tryagain" action="changeemail.php" method=' .
    '"post" autocomplete="on"><fieldset><legend>Change Email</legend>' .
    '<label><b>Password:</b></label><input type="text" placeholder=' .
    '"Enter Password" name="currentpwd" maxlength="30" required>' .
    '<label><b>New Email:</b></label><input type="text" placeholder=' .
    '"Enter New Email Address" name="newemail" maxlength="30" required>' .
    '<div class="clearfix"><button type="button" onclick=' .
    '"window.location.href=\'brickball.php\'" class=' .
    '"cancelbtn">Cancel</button><button type="submit">Submit</button>' .
