...
if ($_SESSION['pword'] == md5($_POST['currentpsw'])) {
    try {
        ...
        md5($_POST['newpsw']) . "' WHERE username='" . 
        $_SESSION['uname'] . "' AND password='" . 
        $_SESSION['pword'] . "'");
        ...
        /* see if they automatically stay logged in via cookies and update 
        the cookie password */
        ...
        echo '<div class="form-response">Password successfully changed!' .
        '<br /><br /><button type="button" onclick=' .
        '"window.location.href=\'brickball.php\'">Play a Game</button>' .
        '</div>';
        
    } catch (PDOException $e) {
        echo '<p class="form-response"><span class="errormsg">' .
        'Oopsy Daisy!  Error: ' . 
        $e->getMessage() . '</span><br /><a class="error-link" href=' .
        '"brickball.php">Go back and play the game</a>.</p>';
    }

    $conn = null;
} else {
    echo '<div class="form-response"><span class="errormsg">The current' .
    ' password entered is incorrect.  Please, try again.</span>' .
    '<form class="tryagain" action="changepword.php" method=' .
    '"post" autocomplete="on"><fieldset><legend>Change Password</legend>' .
    '<label><b>Current Password:</b></label><input type=' .
    '"text" placeholder="Enter Current Password" name=' .
    '"currentpsw" maxlength="30" required><label><b>New Password:</b>' .
    '</label><input type="text" placeholder="Enter New Password" name=' .
    '"newpsw" maxlength="30" required><div class="clearfix">' .
