...
try {
    ...
    // delete the member
    $sql = "DELETE FROM members WHERE username='" . $_SESSION['uname'] . 
    "' AND password='" . $psw . "'";
    ...
    if ($row == null) {
        ...
        echo '<p class="form-response">Membership deleted successfully.' .
        '<br><br><button type="button" onclick="window.location.href=' .
        '\'logout.php\'">Play a Game</button></p>';
    }

}
catch (PDOException $e) {
    echo '<p class="form-response"><span class="errorsmg">Oopsy Daisy! ' .
    ' Error: ' . $e->getMessage() . '</span><a class="error-link" href=' .
